...
RTCM_DEVICE="/dev/serial/by-id/usb-1a86_USB_Serial-if00-port0"
RTCM_SCRIPT="/config/scripts/rtcm_udp_forward.py"

start_rtcm_forwarder() {
    local device="$1"
    local script="$2"

    if [[ ! -f "$script" ]]; then
        echo "[RTCM] Script $script not found, skipping launch."
        return
    fi

    while true; do
        until [[ -e "$device" ]]; do
            echo "[RTCM] Waiting for GNSS base on $device..."
            sleep 1
        done

        echo "[RTCM] Starting RTCM forwarder..."
        python3 "$script" --base "$device" --dest-ip 127.0.0.1
        echo "[RTCM] Forwarder exited, restarting after 1s..."
        sleep 1
    done
}

start_rtcm_forwarder "$RTCM_DEVICE" "$RTCM_SCRIPT" &
...
#Start PixelPilot
     &
#pixelpilot --osd --osd-elements 0 --osd-custom-message --osd-refresh 10 --osd-config /config/scripts/osd.json --screen-mode 1920x1080@60 --dvr-framerate 60 --dvr-fmp4 --dvr-sequenced-files --dvr-template /media/record_%Y-%m-%d_%H-%M-%S.mp4 --disable-vsync &
PID=$!
echo "pixelpilot --osd --osd-elements 0 --osd-custom-message --osd-refresh 10 --osd-config /config/scripts/osd.json --screen-mode $SCREEN_MODE --dvr-framerate $REC_FPS --dvr-fmp4 --dvr-sequenced-files --dvr-template $DVR_PATH/record_%Y-%m-%d_%H-%M-%S.mp4 --disable-vsync --rtp-forward 192.168.2.1:14550" > /tmp/pp.sh
...
ip addr add 192.168.1.11/24 dev enx00e04c68210f
ip link set enx00e04c68210f up
